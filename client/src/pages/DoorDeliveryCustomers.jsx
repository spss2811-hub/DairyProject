...
const DoorDeliveryCustomers = () => {
  ...
  const loadData = async () => {
    try {
      ...
      // Filter only Door Delivery customers
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2>Door Deliver Customers</h2>
        <div className="d-flex gap-2">
            <Button variant="outline-success" onClick={() => navigate('/customer-list')}>
                <FaList className="me-2" /> All Customers
            </Button>
            <Button variant="primary" onClick={() => navigate('/customers', { state: { category: 'Door Delivery' } })}>
                <FaPlus className="me-2" /> Add Door Delivery Customer
            </Button>
        </div>
      </div>
            ...
            <div style={{ maxHeight: '70vh', overflow: 'auto', position: 'relative' }}>
                <Table striped bordered hover className="mb-0" style={{borderCollapse: 'separate', borderSpacing: 0, minWidth: '100%'}}>
                    ...
                    {filteredCustomers.length > 0 ? filteredCustomers.map(c => (
                        ...
                        <tr><td colSpan="9" className="text-center py-4 text-muted">No door delivery customers found</td></tr>
                    )}
                    </tbody>
                </Table>
            </div>
...
export default DoorDeliveryCustomers;
